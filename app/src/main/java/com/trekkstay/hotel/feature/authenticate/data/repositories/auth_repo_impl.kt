...
class AuthRepoImpl(private val remoteDataSource: AuthRemoteDataSource) : AuthRepo {
    ...
    override suspend fun empCreate(
        ...
        return when (val response =
            remoteDataSource.empCreate(email, name, phone, contract, salary)) {
            is Response.Success -> Unit.right()
            is Response.Invalid -> ApiInvalid(
                response.message ?: "Unknown error",
                response.status ?: "-1"
    ...
    override suspend fun changePassword(oldPw: String, newPw: String, newRePw: String): ResultVoid {
        return when (val response = remoteDataSource.changePassword(oldPw, newPw, newRePw)) {
            is Response.Success -> Unit.right()
            is Response.Invalid -> ApiInvalid(
                response.message ?: "Unknown error",
                response.status ?: "-1"
